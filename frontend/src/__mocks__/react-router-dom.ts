...
export const Navigate = ({ to }: { to: string }) =>
  React.createElement('div', {}, `Redirected to ${to}`);

export const BrowserRouter = ({ children }: { children: React.ReactNode }) =>
  React.createElement('div', {}, children);

// MemoryRouter for integration tests - stores navigation history in memory
export const MemoryRouter = ({ children, initialEntries }: { children: React.ReactNode; initialEntries?: string[] }) =>
  React.createElement('div', { 'data-testid': 'memory-router', 'data-initial-entries': initialEntries?.join(',') }, children);
